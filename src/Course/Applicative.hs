...
instance Applicative ExactlyOne where
  ...
  (ExactlyOne f) <*> (ExactlyOne value) = ExactlyOne (f value)
...
instance Applicative List where
  ...
  functions <*> list = flatMap (`map` list) functions
...
f <$$> applicative = pure f <*> applicative
...
instance Applicative Optional where
  ...
  f <*> optionalValue = bindOptional (`mapOptional` optionalValue) f
...
instance Applicative ((->) t) where
  ...
  pure value _ = value
  (<*>) :: (->) t (a -> b) -> (->) t a -> (->) t b
  funTransform <*> funInput =
    \t ->
      let input = funInput t
          f = funTransform t
      in f input
...
lift2 f input1 input2 = f <$$> input1 <*> input2
...
lift3 f input1 input2 input3 = lift2 f input1 input2 <*> input3
...
lift4 f input1 input2 input3 input4 = lift3 f input1 input2 input3 <*> input4
...
(*>) = lift2 (const id)
--_ *> applicative = const <$$> applicative
...
filtering predicate (x:.xs) = let result = lift2 (\isApplicable val -> if isApplicable then val:.Nil else Nil) (predicate x) (pure x) in
                                  lift2 (++)  result (filtering predicate xs)
