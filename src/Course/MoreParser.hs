...
option defaultValue parser = parser ||| valueParser defaultValue
...
oneof eligibleChars = satisfy (`elem` eligibleChars)
...
noneof eligibleChars = satisfy (`notElem` eligibleChars)
...
between parserLeftIgnore parserRightIgnore parser= parserLeftIgnore *> parser <* parserRightIgnore
...
betweenCharTok startCharacter endCharacter = between (charTok startCharacter) (charTok endCharacter)
...
hex = let intParser = readHex <$$> replicateA 4 (satisfy isHexDigit) in
  do optionalChar <- mapOptional chr <$$> intParser
     case optionalChar of Empty -> constantParser UnexpectedEof
                          (Full value) -> valueParser value
...
sepby1 parseContent parseSeparator = convertToList parseContent *+* list (parseSeparator *> parseContent)
...
sepby parseContent parseSeparator = sepby1 parseContent parseSeparator ||| valueParser Nil
...
satisfyAll predicateList = satisfy (and . sequence predicateList)
...
satisfyAny predicateList = satisfy (or . sequence predicateList)
...
betweenSepbyComma beginningChar endingChar parser = betweenCharTok beginningChar endingChar $ sepby parser (is ',')
