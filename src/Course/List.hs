...
map f = foldRight (\input accList -> f input :. accList) Nil
...
filter predicateValue =
  foldRight
    (\inputValue  accList->
       if predicateValue inputValue
         then inputValue :. accList
         else accList)
    Nil
...
seqOptional = foldRight determineOptional (Full Nil)
  where
    determineOptional Empty _ = Empty
    determineOptional _ Empty = Empty
    determineOptional (Full value) (Full subList) = Full (value :. subList)
...
find _ Nil = Empty
find predicate (x:.xs) = if predicate x then Full x else find predicate xs
