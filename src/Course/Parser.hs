...
valueParser value = P (`Result` value)
...
character = P parseCharacter
  where
    parseCharacter Nil = UnexpectedEof
    parseCharacter (firstChar :. remainingChars) = Result remainingChars firstChar
...
bindParser parserCB parser =
  P
    (\input ->
       case parse parser input of
         (Result input' value) -> parse (parserCB value) input'
         UnexpectedEof -> UnexpectedEof
         (ExpectedEof input') -> ExpectedEof input'
         (UnexpectedChar char) -> UnexpectedChar char
         (UnexpectedString string) -> UnexpectedString string)
...
parserA >>> parserB = bindParser (const parserB) parserA
...
leftParser ||| rightParser =
  P
    (\input ->
       let leftParserOutput = parse leftParser input
       in if isErrorResult leftParserOutput
            then parse rightParser input
            else leftParserOutput)
...
list parser = ((:.) <$$> parser <*> list parser) ||| valueParser Nil
...
satisfy predicate =
  bindParser
    (\value ->
       if predicate value
         then valueParser value
         else unexpectedCharParser value)
    character
...
is :: Char -> Parser Char -- Creates a new parser
is givenChar = satisfy (== givenChar)
...
ageParser =
  bindParser
    (\stringAge ->
       case read stringAge of
         Empty -> constantParser (UnexpectedString stringAge)
         Full parsedAge ->
           if headOr '0' stringAge == '0'
             then constantParser (UnexpectedString stringAge)
             else valueParser parsedAge)
    (list1 digit)
...
convertToList :: Parser a -> Parser (List a)
convertToList parser = (:. Nil) <$> parser

firstNameParser :: Parser Chars
firstNameParser = convertToList upper *+* list lower
...
surnameParser = convertToList upper *+* thisMany 5 lower *+* list lower
...
phoneParser = (convertToList digit *+* phoneBodyParser) <* is '#'
...
personParser = do
  thisAge <- ageParser
  thisFirstName <- firstNameParser
  thisSurname <- surnameParser
  thisSmoker <- smokerParser
  thisPhone <- phoneParser
  return (Person thisAge thisFirstName thisSurname thisSmoker thisPhone)
...
instance Functor Parser where
  ...
  f <$> parser = bindParser (valueParser . f) parser
